...
function ConvertTo-SafeHtml {
    <#
    .SYNOPSIS
        Escapes '&', '<', '>', '"' and '\'' for safe interpolation into HTML text nodes and
        into attribute values quoted with EITHER single or double quotes.
    .DESCRIPTION
        ...
        BOTH QUOTE CHARACTERS ARE ESCAPED, so the output is safe in all three positions a
        caller might use it: a text node, a double-quoted attribute, and a single-quoted
        attribute. This matters because the HTML generated by this script quotes its
        attributes with SINGLE quotes (e.g. data-label='...'), so escaping only '"' would
        leave a single-quoted attribute breakable by a lone apostrophe in customer data - a
        UNC path or GPO name containing one could close the attribute early and inject
        markup. No such value is interpolated into an attribute today (every attribute is
        built from this script's own hardcoded strings, and all customer data lands in text
        nodes), so this is defence in depth rather than a fix for a live hole - but it means
        a future caller cannot introduce the hole by doing the obvious thing.

        '&#39;' is used rather than '&apos;': the named entity is not defined in HTML 4 and
        is unreliable in legacy parsers, while the numeric reference is valid everywhere.
    #>
