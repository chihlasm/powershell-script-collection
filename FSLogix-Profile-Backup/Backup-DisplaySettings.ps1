...
.EXAMPLE
    ...
    Backs up display settings from current user's registry
.EXAMPLE
    .\Backup-DisplaySettings.ps1 -NTUserPath "E:\Profile\NTUSER.DAT" -OutputPath "C:\Backup\Display.reg"
    Backs up display settings from a mounted FSLogix profile
#>
...
try {
    ...
    if ($NTUserPath -and (Test-Path $NTUserPath)) {
        ...
        Write-Host "Exporting display settings from mounted profile..."
    } else {
        if ($NTUserPath) {
            Write-Warning "NTUSER.DAT not found at: $NTUserPath"
            Write-Warning "Falling back to current user's registry"
        }
        Write-Host "Exporting display settings from current user's registry..."
    }

    # Display-related registry keys to export
    $regKeys = @(
        "Software\Microsoft\Windows\CurrentVersion\Display",
        "Software\Microsoft\Windows\CurrentVersion\Explorer\Dpi",
