...
- **Who Gets What (Matrix)** - Interactive group-vs-drive-letter matrix in the HTML report showing every group's mapping at a glance, with expand-to-users to see the real AD membership behind each group (when `-CheckGroupOverlap` was used)
...
| `*-Matrix.csv` | Who Gets What matrix: rows = groups, columns = drive letters, cell = UNC path(s) |
