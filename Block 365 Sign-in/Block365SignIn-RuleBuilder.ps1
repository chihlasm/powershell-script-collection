<#
.SYNOPSIS
    Creates the Entra Connect sync rule that enforces Block Cloud Sign-In.
.DESCRIPTION
    CloudSignInMgr.ps1 marks users by writing msDS-cloudExtensionAttribute10 =
    "BlockCloudSignIn" in Active Directory. That marker does nothing by itself. This
    script creates the custom inbound synchronization rule inside Entra Connect that
    turns the marker into enforcement, by flowing it to cloudFiltered so the user stops
    syncing to Entra ID and can no longer sign in to Microsoft 365.
...
.EXAMPLE
    .\Block365SignIn-RuleBuilder.ps1 -EntraConnectServer AADC02
...
.EXAMPLE
    .\Block365SignIn-RuleBuilder.ps1 -EntraConnectServer AADC02 -WhatIf
...
$definition   = Get-EntraConnectRuleDefinition -Key 'BlockCloudSignIn'
...
Write-Host ''
Write-Host '=== Block 365 Sign-In: Entra Connect rule builder ===' -ForegroundColor White
Write-Host ''
...
# "Block cloud sign-in - " and a connector match against every AD connector.
$targetDomain = $null
try {
...
$existing = Test-EntraConnectRule -RuleKey 'BlockCloudSignIn' -ComputerName $server `
...
if (-not $result.Success) {
    ...
    Write-Host '  procedure and is worth doing: a cloudFiltered rule removes objects from' -ForegroundColor Gray
    Write-Host '  Entra ID, so an unexpected match here is important to catch first.' -ForegroundColor Gray
    Write-Host ''
}
...
try {
    ...
    $logFile = Join-Path $OutputPath "Block365SignIn-RuleBuilder_$(Get-Date -Format 'yyyy-MM-dd_HHmmss').log"
